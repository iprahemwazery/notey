...
import 'package:notey/features/settings/domain/usecases/get_trash_retention_days.dart';
import 'package:notey/features/settings/domain/usecases/set_trash_retention_days.dart';
...
class TrashRetentionTile extends StatefulWidget {
  TrashRetentionTile({super.key, SettingsRepository? repository})
...
class _TrashRetentionTileState extends State<TrashRetentionTile> {
  ...
  void initState() {
    super.initState();
    GetTrashRetentionDays(widget.repository)().then((v) {
  ...
  Future<void> _pickRetention() async {
    ...
    if (picked != null && picked != _days) {
      await SetTrashRetentionDays(widget.repository)(picked);
