...
import 'package:notey/features/auth/domain/usecases/clear_lockout.dart';
import 'package:notey/features/auth/domain/usecases/get_remaining_lockout.dart';
import 'package:notey/features/auth/domain/usecases/record_failed_attempt.dart';
import 'package:notey/features/auth/domain/usecases/start_lockout_if_needed.dart';
...
class AuthCubit extends Cubit<AuthState> {
  AuthCubit({
    ...
    AuthLockoutRepository? lockoutRepository,
  }) : _lock = lockController,
       _bio = biometricService,
       _lockout = lockoutRepository ?? AuthLockoutRepositoryImpl(),
       super(const AuthState()) {
    _remainingLockout = GetRemainingLockout(_lockout);
    _recordAttempt = RecordFailedAttempt(_lockout);
    _startLockout = StartLockoutIfNeeded(_lockout);
    _clearLockout = ClearLockout(_lockout);
  }

  final AppLockController _lock;
  final BiometricService _bio;
  final AuthLockoutRepository _lockout;
  late final GetRemainingLockout _remainingLockout;
  late final RecordFailedAttempt _recordAttempt;
  late final StartLockoutIfNeeded _startLockout;
  late final ClearLockout _clearLockout;
  ...
  Future<void> _checkLockout() async {
    ...
    if (isClosed) return;
    if (remaining != null) {
      _startLockoutTimer(remaining.inMilliseconds);
    }
  }
  ...
  Future<void> _handleFailedAttempt() async {
    final attempts = await _recordAttempt();
    if (isClosed) return;
    final lockout = await _startLockout(failedAttempts: attempts);
    if (isClosed) return;

    if (lockout != null) {
      _startLockoutTimer(lockout.inMilliseconds);
    } else {
